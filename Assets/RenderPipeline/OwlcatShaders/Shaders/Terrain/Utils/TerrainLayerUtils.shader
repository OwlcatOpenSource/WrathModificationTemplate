// Unity built-in shader source. Copyright (c) 2016 Unity Technologies. MIT license (see license.txt)
    ...
    Shader "Hidden/Owlcat/Terrain/TerrainLayerUtils" {
    ...
    SubShader {
        ...
        Pass    // Copy the R channel of the input into a specific channel in the output
        {
            ...
            float4 SetLayer(v2f i) : SV_Target
            {
                ...
                float brushShape = BRUSH_STRENGTH * UnpackHeightmap(tex2D(_BrushTex, i.texcoord));
                return lerp(alphaMap, _LayerMask * newAlpha + (1 - _LayerMask) * alphaMap, brushShape);
                if (brushShape > 0)
                {
                    return _LayerMask * newAlpha + (1 - _LayerMask) * alphaMap;
                }
                else
                {
                    return alphaMap;
