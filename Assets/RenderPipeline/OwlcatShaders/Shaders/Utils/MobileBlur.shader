...
Shader "Hidden/MobileBlur" {
	...
	CGINCLUDE
		...
		v2f_withBlurCoordsSGX vertBlurHorizontalSGX (appdata_img v)
		{
			...
			o.offs0 = UnityStereoScreenSpaceUVAdjust(v.texcoord.xyxy + offsetMagnitude * half4(-3.0h, 0.0h, 3.0h, 0.0h), _MainTex_ST);
			o.offs1 = UnityStereoScreenSpaceUVAdjust(v.texcoord.xyxy + offsetMagnitude * half4(-2.0h, 0.0h, 2.0h, 0.0h), _MainTex_ST);
			o.offs2 = UnityStereoScreenSpaceUVAdjust(v.texcoord.xyxy + offsetMagnitude * half4(-1.0h, 0.0h, 1.0h, 0.0h), _MainTex_ST);
		...
		v2f_withBlurCoordsSGX vertBlurVerticalSGX (appdata_img v)
		{
			...
			o.offs0 = UnityStereoScreenSpaceUVAdjust(v.texcoord.xyxy + offsetMagnitude * half4(0.0h, -3.0h, 0.0h, 3.0h), _MainTex_ST);
			o.offs1 = UnityStereoScreenSpaceUVAdjust(v.texcoord.xyxy + offsetMagnitude * half4(0.0h, -2.0h, 0.0h, 2.0h), _MainTex_ST);
			o.offs2 = UnityStereoScreenSpaceUVAdjust(v.texcoord.xyxy + offsetMagnitude * half4(0.0h, -1.0h, 0.0h, 1.0h), _MainTex_ST);
		...
		half4 fragBlurSGX ( v2f_withBlurCoordsSGX i ) : SV_Target
		{
			...
			half4 color = tex2D(_MainTex, i.uv) * curve4[3];
  			...
  			for( int l = 0; l < 3; l++ )  
  			{   
				half4 tapA = tex2D(_MainTex, i.offs1.xy);
				half4 tapB = tex2D(_MainTex, i.offs1.zw); 
